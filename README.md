...
<details><summary>접기/펼치기</summary>
    ...
    <img width="364" height="149" alt="image" src="https://github.com/user-attachments/assets/63ab7c6d-bab9-4c5e-8f12-7a2190916b5e" />

    
맵 반복에 대한 문제
백그라운드는 고정적으로 움직이는 반면, 스테이지는 랜덤값을 주어 다음으로 이동해야됨으로 해당 문제를 해결하는데는 상관이 없었지만, 최대 스테이지에 따른 고정 맵 추가를 넣음으로 문제가 발생하였다

해당 문제를 해결하기위해 전체 맵 수를 확인하고 
현재 부딪힌 BackGround가 HP를 회복시키는 맵이면 
해당 맵을 제외하여 맵 카운트를 올렸고
MapCheck()에서 마지막 맵의 bool값을 true로 변경하여
스테이지를 불러오는 부분에서 bool값이 true일경우 회복맵을 추가했다
